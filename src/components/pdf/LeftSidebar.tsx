      ...
      <aside className="flex flex-col h-screen w-[340px] min-w-[280px] border-r border-slate-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 p-4">
        ...
        <div className="flex-1 overflow-y-auto space-y-4">
          {items.map((item, index) => {
            // Determine blur: blur the current word and all words after it that aren't checked
            const shouldBlur = !item.checked && index >= activeIndex && activeIndex !== -1;
            ...
            return (
              ...
              >
                <p className="text-sm font-medium text-slate-900 dark:text-zinc-100">
                  {item.checked ? 'Done' : item.id === activeItemId ? 'Current' : 'Next'}:{' '}
                  <span
